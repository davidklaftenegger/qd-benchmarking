...
void cpplock_delegate(AgnosticDXLock* x, void (*delgateFun)(int, int *), int data) {
	...
	//l->delegate_n([delgateFun, data]() {delgateFun(data, nullptr);});
}
#if 1
int cpplock_delegate_and_wait(AgnosticDXLock* x, void (*delgateFun)(int, int *), int data) {
	...
	//l->delegate_n([delgateFun, data, &resp, &flag]() { delgateFun(data, &resp);  flag.store(true, std::memory_order_release); });
