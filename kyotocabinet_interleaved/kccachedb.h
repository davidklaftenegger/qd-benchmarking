  ...
  class Cursor : public BasicDB::Cursor {
   ...
   public:
    ...
    virtual ~Cursor() {
      _assert_(true);
      if (!db_) return;
      /* return value is dummy, needed to wait on it */
      struct hide {
      static bool taskfun(std::tuple<Cursor*>* tuple) {
	  auto self = std::get<0>(*tuple);
	  if (!self->db_) return false;
	  self->db_->curs_.remove(self);
	  return true;
      }};
      
      result<bool> res;
      using task_t = delegatable<decltype(hide::taskfun), bool, decltype(this)>;
      new (res.space) task_t(hide::taskfun, &res, this);
      task_t* task = reinterpret_cast<task_t*>(&res.space);
      db_->mlock_.lock_delegate_del(task);
      res.get(); // just wait for it to happen
    }
    /**
     ...
     */
    void accept(result<bool>* res, Error* err, Visitor* visitor, bool writable = true, bool step = false) {
      _assert_(visitor);
      struct hide {
      static bool taskfun_ff(std::tuple<Cursor*, decltype(err), decltype(visitor)>* tuple) { return taskfun_i(tuple, false, false); }
      static bool taskfun_ft(std::tuple<Cursor*, decltype(err), decltype(visitor)>* tuple) { return taskfun_i(tuple, false, true); }
      static bool taskfun_tf(std::tuple<Cursor*, decltype(err), decltype(visitor)>* tuple) { return taskfun_i(tuple, true, false); }
      static bool taskfun_tt(std::tuple<Cursor*, decltype(err), decltype(visitor)>* tuple) { return taskfun_i(tuple, true, true); }
      static bool taskfun_i(std::tuple<Cursor*, decltype(err), decltype(visitor)>* tuple, bool writable, bool step) {
      auto self = std::get<0>(*tuple);
      auto err = std::get<1>(*tuple);
      auto visitor = std::get<2>(*tuple);
      if (self->db_->omode_ == 0) {
        self->db_->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
        return false;
      }
      if (writable && !(self->db_->omode_ & OWRITER)) {
        self->db_->set_error(_KCCODELINE_, Error::NOPERM, "permission denied", err);
        return false;
      }
      if (self->sidx_ < 0 || !self->rec_) {
        self->db_->set_error(_KCCODELINE_, Error::NOREC, "no record", err);
        return false;
      }
      uint32_t rksiz = self->rec_->ksiz & KSIZMAX;
      char* dbuf = (char*)self->rec_ + sizeof(*self->rec_);
      const char* rvbuf = dbuf + rksiz;
      size_t rvsiz = self->rec_->vsiz;
      char* zbuf = NULL;
      size_t zsiz = 0;
      if (self->db_->comp_) {
        zbuf = self->db_->comp_->decompress(rvbuf, rvsiz, &zsiz);
        if (zbuf) {
          rvbuf = zbuf;
          rvsiz = zsiz;
        }
      ...
      if (vbuf == Visitor::REMOVE) {
        uint64_t hash = self->db_->hash_record(dbuf, rksiz) / SLOTNUM;
        Slot* slot = self->db_->slots_ + self->sidx_;
        Repeater repeater(Visitor::REMOVE, 0);
        self->db_->accept_impl(slot, hash, dbuf, rksiz, &repeater, self->db_->comp_, false, err);
      } else if (vbuf == Visitor::NOP) {
        if (step) self->step_impl(err);
      } else {
        uint64_t hash = self->db_->hash_record(dbuf, rksiz) / SLOTNUM;
        Slot* slot = self->db_->slots_ + self->sidx_;
...
//      });
      ...
      if(!writable && !step) {
	using task_t = delegatable<decltype(hide::taskfun_ff), bool, decltype(this), decltype(err), decltype(visitor)>;
	new (res->space) task_t(hide::taskfun_ff, res, this, err, visitor);
	task_t* task = reinterpret_cast<task_t*>(&res->space);
	db_->mlock_.lock_delegate_del(task);
	return;
      } else if(!writable && step) {
	using task_t = delegatable<decltype(hide::taskfun_ft), bool, decltype(this), decltype(err), decltype(visitor)>;
	new (res->space) task_t(hide::taskfun_ft, res, this, err, visitor);
	task_t* task = reinterpret_cast<task_t*>(&res->space);
	db_->mlock_.lock_delegate_del(task);
	return;
      } else if(writable && !step) {
	using task_t = delegatable<decltype(hide::taskfun_tf), bool, decltype(this), decltype(err), decltype(visitor)>;
	new (res->space) task_t(hide::taskfun_tf, res, this, err, visitor);
	task_t* task = reinterpret_cast<task_t*>(&res->space);
	db_->mlock_.lock_delegate_del(task);
	return;
      } else { // == if(writable && step) {
	using task_t = delegatable<decltype(hide::taskfun_tt), bool, decltype(this), decltype(err), decltype(visitor)>;
	new (res->space) task_t(hide::taskfun_tt, res, this, err, visitor);
	task_t* task = reinterpret_cast<task_t*>(&res->space);
	db_->mlock_.lock_delegate_del(task);
	return;
      } 
...
//      return f.get();
    ...
    bool accept(Visitor* visitor, bool writable = true, bool step = false) {
      Error* err = db_->error_.get();
      result<bool> r;
      this->accept(&r, err, visitor, writable, step);
      return r.get();
    }
    ...
    bool jump() {
      ...
      Error* err = db_->error_.get();
      struct hide {
      static bool taskfun(std::tuple<Cursor*, decltype(err)>* tuple) {
      auto self = std::get<0>(*tuple);
      auto err = std::get<1>(*tuple);
      if (self->db_->omode_ == 0) {
        self->db_->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
      ...
      for (int32_t i = 0; i < SLOTNUM; i++) {
        Slot* slot = self->db_->slots_ + i;
        if (slot->first) {
          self->sidx_ = i;
          self->rec_ = slot->first;
          return true;
        }
      }
      self->db_->set_error(_KCCODELINE_, Error::NOREC, "no record", err);
      ...
      result<bool> res;
      using task_t = delegatable<decltype(hide::taskfun), bool, decltype(this), decltype(err)>;
      new (res.space) task_t(hide::taskfun, &res, this, err);
      task_t* task = reinterpret_cast<task_t*>(&res.space);
      db_->mlock_.lock_delegate_del(task);
      return res.get();
    }
     ...
     * @return true on success, or false on failure.
     */
    void jump(result<bool>* res, Error* err, const char* kbuf, size_t ksiz_p) {
      _assert_(kbuf && ksiz_p <= MEMMAXSIZ);
      struct hide {
      static bool taskfun(std::tuple<Cursor*, decltype(err), decltype(kbuf), decltype(ksiz_p)>* tuple) {
      auto self = std::get<0>(*tuple);
      auto err = std::get<1>(*tuple);
      auto kbuf = std::get<2>(*tuple);
      auto ksiz = std::get<3>(*tuple);
      if (self->db_->omode_ == 0) {
        self->db_->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
        return false;
      }
//      auto ksiz = ksiz_p;
      ...
      uint64_t hash = self->db_->hash_record(kbuf, ksiz);
      int32_t sidx = hash % SLOTNUM;
      hash /= SLOTNUM;
      Slot* slot = self->db_->slots_ + sidx;
      size_t bidx = hash % slot->bnum;
      Record* rec = slot->buckets[bidx];
      Record** entp = slot->buckets + bidx;
      uint32_t fhash = self->db_->fold_hash(hash) & ~KSIZMAX;
      while (rec) {
        ...
        if (fhash > rhash) {
          ...
          int32_t kcmp = self->db_->compare_keys(kbuf, ksiz, dbuf, rksiz);
          if (kcmp < 0) {
            entp = &rec->left;
            rec = rec->left;
          } else if (kcmp > 0) {
      ...
      using task_t = delegatable<decltype(hide::taskfun), bool, decltype(this), decltype(err), decltype(kbuf), decltype(ksiz_p)>;
      new (res->space) task_t(hide::taskfun, res, this, err, kbuf, ksiz_p);
      task_t* task = reinterpret_cast<task_t*>(&res->space);
      db_->mlock_.lock_delegate_del(task);
      return;
    }
    bool jump(const char* kbuf, size_t ksiz_p) {
      Error* err = db_->error_.get();
      result<bool> res;
      jump(&res, err, kbuf, ksiz_p);
      return res.get();
    }
    /**
     * Jump the cursor to a record for forward scan.
    ...
    bool jump_back() {
      _assert_(true);
      Error* err = db_->error_.get();
      std::packaged_task<bool()> task( [this, err] () {
      if (db_->omode_ == 0) {
        db_->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
        return false;
      }
      db_->set_error(_KCCODELINE_, Error::NOIMPL, "not implemented", err);
      return false;
      });
      auto f = task.get_future();
      db_->mlock_.lock_delegate(&task);
    ...
    bool jump_back(const char* kbuf, size_t ksiz) {
      ...
      Error* err = db_->error_.get();
      std::packaged_task<bool()> task( [this, err, kbuf, ksiz] () {
      if (db_->omode_ == 0) {
        db_->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
        return false;
      }
    ...
    bool jump_back(const std::string& key) {
      _assert_(true);
      Error* err = db_->error_.get();
      std::packaged_task<bool()> task( [this, err, key] () {
      if (db_->omode_ == 0) {
        db_->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
        return false;
      }
     ...
     * @return true on success, or false on failure.
     */
    void step(result<bool>* res, Error* error) {
      _assert_(true);
      struct hide {
      static bool taskfun(std::tuple<Cursor*, decltype(error)>* tuple) {
      auto self = std::get<0>(*tuple);
      auto error = std::get<1>(*tuple);
      if (self->db_->omode_ == 0) {
        self->db_->set_error(_KCCODELINE_, Error::INVALID, "not opened", error);
        return false;
      }
      if (self->sidx_ < 0 || !self->rec_) {
        self->db_->set_error(_KCCODELINE_, Error::NOREC, "no record", error);
        return false;
      }
      bool err = false;
      if (!self->step_impl(error)) err = true;
      return !err;
      }};
      using task_t = delegatable<decltype(hide::taskfun), bool, decltype(this), decltype(error)>;
      new (res->space) task_t(hide::taskfun, res, this, error);
      task_t* task = reinterpret_cast<task_t*>(&res->space);
      db_->mlock_.lock_delegate_del(task);
      return;
    }
   ...
   private:
    ...
    bool step_impl(Error* err) {
      ...
      if (!rec_) {
        ...
        db_->set_error(_KCCODELINE_, Error::NOREC, "no record", err);
        sidx_ = -1;
        rec_ = NULL;
        return false;
      }
  ...
  bool accept(const char* kbuf, size_t ksiz, Visitor* visitor, bool writable = true) {
    ...
    slot->lock.lock();
    accept_impl(slot, hash, kbuf, ksiz, visitor, comp_, rttmode_, error_.get());
    slot->lock.unlock();
    return true;
  }
  /**
  ...
  bool accept_bulk(const std::vector<std::string>& keys, Visitor* visitor,
                   bool writable = true) {
    _assert_(visitor);
    Error* err = error_.get();
    ScopedRDLock lock(&mlock_);
    if (omode_ == 0) {
      set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
      return false;
    }
    if (writable && !(omode_ & OWRITER)) {
      set_error(_KCCODELINE_, Error::NOPERM, "permission denied", err);
      return false;
    }
    ...
    for (size_t i = 0; i < knum; i++) {
      ...
      accept_impl(slot, rkey->hash, rkey->kbuf, rkey->ksiz, visitor, comp_, rttmode_, err);
    }
    sit = sidxs.begin();
    sitend = sidxs.end();
    while (sit != sitend) {
  ...
  bool iterate(Visitor *visitor, bool writable = true, ProgressChecker* checker = NULL) {
    _assert_(visitor);
    Error* err = error_.get();
    std::cout << "ey alta checka: " << checker << std::endl;
    struct hide {
    static bool taskfun_f(std::tuple<CacheDB*, decltype(err), decltype(visitor), decltype(checker)>* tuple) { return taskfun_i(tuple, false); }
    static bool taskfun_t(std::tuple<CacheDB*, decltype(err), decltype(visitor), decltype(checker)>* tuple) { return taskfun_i(tuple, true); }
    static bool taskfun_i(std::tuple<CacheDB*, decltype(err), decltype(visitor), decltype(checker)>* tuple, bool writable) {
    auto self = std::get<0>(*tuple);
    auto err = std::get<1>(*tuple);
    auto visitor = std::get<2>(*tuple);
//    auto writable = std::get<3>(*tuple);
    auto checker = std::get<3>(*tuple);
    if (self->omode_ == 0) {
      self->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
      return false;
    }
    if (writable && !(self->omode_ & OWRITER)) {
      self->set_error(_KCCODELINE_, Error::NOPERM, "permission denied", err);
      return false;
    }
    ScopedVisitor svis(visitor);
    int64_t allcnt = self->count_impl();
    if (checker && !checker->check("iterate", "beginning", 0, allcnt)) {
      self->set_error(_KCCODELINE_, Error::LOGIC, "checker failed", err);
      return false;
    }
    int64_t curcnt = 0;
    for (int32_t i = 0; i < SLOTNUM; i++) {
      Slot* slot = self->slots_ + i;
      ...
      while (rec) {
        ...
        size_t zsiz = 0;
        if (self->comp_) {
          zbuf = self->comp_->decompress(rvbuf, rvsiz, &zsiz);
          if (zbuf) {
            rvbuf = zbuf;
            rvsiz = zsiz;
          }
        ...
        delete[] zbuf;
        if (vbuf == Visitor::REMOVE) {
          uint64_t hash = self->hash_record(dbuf, rksiz) / SLOTNUM;
          Repeater repeater(Visitor::REMOVE, 0);
          self->accept_impl(slot, hash, dbuf, rksiz, &repeater, self->comp_, false, err);
        } else if (vbuf != Visitor::NOP) {
          uint64_t hash = self->hash_record(dbuf, rksiz) / SLOTNUM;
          Repeater repeater(vbuf, vsiz);
          self->accept_impl(slot, hash, dbuf, rksiz, &repeater, self->comp_, false, err);
    ...
    self->trigger_meta(MetaTrigger::ITERATE, "iterate");
    return true;
    }};
    result<bool> res;
    if(writable) {
        using task_t = delegatable<decltype(hide::taskfun_t), bool, decltype(this), decltype(err), decltype(visitor), decltype(checker)>;
        new (res.space) task_t(hide::taskfun_t, &res, this, err, visitor, checker);
        task_t* task = reinterpret_cast<task_t*>(&res.space);
        mlock_.lock_delegate_del(task);
	return res.get();
    } else {
        using task_t = delegatable<decltype(hide::taskfun_f), bool, decltype(this), decltype(err), decltype(visitor), decltype(checker)>;
        new (res.space) task_t(hide::taskfun_f, &res, this, err, visitor, checker);
        task_t* task = reinterpret_cast<task_t*>(&res.space);
        mlock_.lock_delegate_del(task);
	return res.get();
    }
   ...
   * @param error pointer to the error code field
   */
  void set_error(const char* file, int32_t line, const char* func,
                 Error::Code code, const char* message, Error* error) {
  ...
  void set_error(const char* file, int32_t line, const char* func,
    ...
    set_error(file, line, func, code, message, error_.get());
  }
  /**
  ...
  bool open(const std::string& path, uint32_t mode = OWRITER | OCREATE) {
    _assert_(true);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, path, mode] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
  ...
  bool occupy(bool writable = true, FileProcessor* proc = NULL) {
    _assert_(true);
    if(writable) {
      Error* error = error_.get();
      std::packaged_task<bool()> task( [this, error, writable, proc] () {
      bool err = false;
      if (proc && !proc->process(path_, count_impl(), size_impl())) {
	set_error(_KCCODELINE_, Error::LOGIC, "processing failed", error);
	err = true;
      }
...
*/
  enum transaction_start {fail, success, retry};
  bool begin_transaction(bool hard = false) {
    _assert_(true);
    uint32_t wcnt = 0;
    while (true) {
      Error* err = error_.get();
      struct hide {
      static transaction_start taskfun(std::tuple<CacheDB*, decltype(err)>* tuple) {
	auto self = std::get<0>(*tuple);
	auto err = std::get<1>(*tuple);
	/* your code here */
	if (self->omode_ == 0) {
	...
	if (!(self->omode_ & OWRITER)) {
	  self->set_error(_KCCODELINE_, Error::NOPERM, "permission denied", err);
	  return fail;
	}
	if (!self->tran_) {
	  self->tran_ = true;
	  self->trigger_meta(MetaTrigger::BEGINTRAN, "begin_transaction");
	  return success;
	}
	return retry;
      }};
      result<transaction_start> res;
      using task_t = delegatable<decltype(hide::taskfun), transaction_start, decltype(this), decltype(err)>;
      new (res.space) task_t(hide::taskfun, &res, this, err);
      task_t* task = reinterpret_cast<task_t*>(&res.space);
      mlock_.lock_delegate_del(task);
  ...
  bool begin_transaction_try(bool hard = false) {
    ...
    Error* err = error_.get();
    struct hide {
    static bool taskfun(std::tuple<CacheDB*, decltype(err), decltype(hard)>* tuple) {
    auto self = std::get<0>(*tuple);
    auto err = std::get<1>(*tuple);
//    auto hard = std::get<2>(*tuple); // not used at all
    if (self->omode_ == 0) {
      self->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
      return false;
    }
    ...
    if (self->tran_) {
      self->set_error(_KCCODELINE_, Error::LOGIC, "competition avoided", err);
      return false;
    }
    self->tran_ = true;
    self->trigger_meta(MetaTrigger::BEGINTRAN, "begin_transaction_try");
    return true;
    }};
    result<bool> res;
    using task_t = delegatable<decltype(hide::taskfun), bool, decltype(this), decltype(err), decltype(hard)>;
    new (res.space) task_t(hide::taskfun, &res, this, err, hard);
    task_t* task = reinterpret_cast<task_t*>(&res.space);
    mlock_.lock_delegate_del(task);
    return res.get();
  }
  ...
  bool end_transaction(bool commit = true) {
    ...
    Error* err = error_.get();
    struct hide {
    static bool taskfun(std::tuple<CacheDB*, decltype(err), decltype(commit)>* tuple) {
    auto self = std::get<0>(*tuple);
    auto err = std::get<1>(*tuple);
    auto commit = std::get<2>(*tuple);
    if (self->omode_ == 0) {
      self->set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
      return false;
    }
    if (!self->tran_) {
      self->set_error(_KCCODELINE_, Error::INVALID, "not in transaction", err);
      return false;
    }
    if (!commit) self->disable_cursors();
    for (int32_t i = 0; i < SLOTNUM; i++) {
      if (!commit) self->apply_slot_trlogs(self->slots_ + i, err);
      self->slots_[i].trlogs.clear();
      self->adjust_slot_capacity(self->slots_ + i, err);
    }
    self->tran_ = false;
    self->trigger_meta(commit ? MetaTrigger::COMMITTRAN : MetaTrigger::ABORTTRAN, "end_transaction");
    return true;
    }};
    result<bool> res;
    using task_t = delegatable<decltype(hide::taskfun), bool, decltype(this), decltype(err), decltype(commit)>;
    new (res.space) task_t(hide::taskfun, &res, this, err, commit);
    task_t* task = reinterpret_cast<task_t*>(&res.space);
    mlock_.lock_delegate_del(task);
    return res.get();
  }
  ...
  bool status(std::map<std::string, std::string>* strmap) {
    _assert_(strmap);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, strmap] () {
    if (omode_ == 0) {
      set_error(_KCCODELINE_, Error::INVALID, "not opened", err);
      return false;
    }
  ...
  bool tune_logger(Logger* logger, uint32_t kinds = Logger::WARN | Logger::ERROR) {
    _assert_(logger);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, logger, kinds] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
  ...
  bool tune_meta_trigger(MetaTrigger* trigger) {
    _assert_(trigger);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, trigger] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
  ...
  bool tune_options(int8_t opts) {
    _assert_(true);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, opts] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
  ...
  bool tune_buckets(int64_t bnum) {
    _assert_(true);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, bnum] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
  ...
  bool tune_compressor(Compressor* comp) {
    _assert_(comp);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, comp] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
  ...
  bool cap_count(int64_t count) {
    _assert_(true);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, count] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
  ...
  bool cap_size(int64_t size) {
    _assert_(true);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, size] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
   ...
   */
  void switch_rotation(result<bool>* res, bool rttmode) {
    _assert_(true);
    struct hide {
    static bool taskfun(std::tuple<CacheDB*, decltype(rttmode)>* tuple) {
    auto self = std::get<0>(*tuple);
    auto rttmode = std::get<1>(*tuple);
    self->rttmode_ = rttmode;
    return true;
    }};
    using task_t = delegatable<decltype(hide::taskfun), bool, decltype(this), decltype(rttmode)>;
    new (res->space) task_t(hide::taskfun, res, this, rttmode);
    task_t* task = reinterpret_cast<task_t*>(&res->space);
    mlock_.lock_delegate_del(task);
    return;
  }
 ...
 protected:
  ...
  bool tune_type(int8_t type) {
    _assert_(true);
    Error* err = error_.get();
    std::packaged_task<bool()> task( [this, err, type] () {
    if (omode_ != 0) {
      set_error(_KCCODELINE_, Error::INVALID, "already opened", err);
      return false;
    }
 ...
 private:
  ...
  void accept_impl(Slot* slot, uint64_t hash, const char* kbuf, size_t ksiz, Visitor* visitor,
                   Compressor* comp, bool rtt, Error* err) {
      ...
      if (fhash > rhash) {
        ...
        if (kcmp < 0) {
          ...
          if (vbuf == Visitor::REMOVE) {
            ...
            if (!curs_.empty()) escape_cursors(rec, err);
            ...
            if (adj) adjust_slot_capacity(slot, err);
          }
          return;
        }
      }
    ...
    if (vbuf != Visitor::NOP && vbuf != Visitor::REMOVE) {
      ...
      *entp = rec;
      ...
      slot->last = rec;
      slot->count++;
      /* read_lock insufficiently protects here */
      //if (!tran_) adjust_slot_capacity(slot, err);
      delete[] zbuf;
    }
  }
  /**
   ...
   * @param slot the slot table.
   */
  void apply_slot_trlogs(Slot* slot, Error* err) {
    _assert_(slot);
    ...
    while (it != itbeg) {
      ...
      if (it->full) {
        ...
        accept_impl(slot, hash, kbuf, ksiz, &setter, NULL, false, err);
      } else {
        Remover remover;
        accept_impl(slot, hash, kbuf, ksiz, &remover, NULL, false, err);
      }
    }
  }
  /**
   ...
   * @param slot the slot table.
   */
  void adjust_slot_capacity(Slot* slot, Error* err) {
    _assert_(slot);
    if ((slot->count > slot->capcnt || slot->size > slot->capsiz) && slot->first) {
      ...
      Remover remover;
      accept_impl(slot, hash, dbuf, rksiz, &remover, NULL, false, err);
   ...
   */
  void escape_cursors(Record* rec, Error* err) {
    _assert_(rec);
    ...
    while (cit != citend) {
      Cursor* cur = *cit;
      if (cur->rec_ == rec) cur->step_impl(err);
      ++cit;
    }
  }
  /**
