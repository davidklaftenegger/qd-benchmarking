...
/* pinning-related definitions*/
#define INCLUDE_NO_LOCK
#include "lock_impl.h"

#define RESULT_CONTEXTS 128
...
static void dberrprinte(kc::BasicDB* db, int32_t line, const char* func, kc::BasicDB::Error* err) {
	std::cout << err->code() << std::flush << " " << err->name() << std::flush << " " << err->message() << std::endl;
  oprintf("%s: %d: %s: %s: %d: %s: %s\n",
          g_progname, line, func, db->path().c_str(), err->code(), err->name(), err->message());
}
...
struct errordata {
	...
	kc::BasicDB::Error::Code ok_code;
	kc::CacheDB* db;
	int32_t line;
	const char* func;
	
	void set_all_ok(kc::CacheDB* d, int32_t l, const char* f) {
		all_ok = true;
		db = d;
		line = l;
		func = f;
	...
	void set(kc::BasicDB::Error::Code ok, kc::CacheDB* d, int32_t l, const char* f) {
		all_ok = false;
		ok_code = ok;
		db = d;
		line = l;
	...
	bool check_ok(kc::BasicDB::Error* err) {
		if(*err == ok_code) {
			return true;
		} else {
			dberrprinte(db, line, func, err);
			return false;
		}
	}
	bool check_ok() {
		if(all_ok || error == ok_code) {
			return true;
		} else {
			dberrprinte(db, line, func, &error);
			return false;
		}
	}
};
...
static int32_t procwicked(int64_t rnum, int32_t thnum, int32_t itnum,
  ...
  for (int32_t itcnt = 1; itcnt <= itnum; itcnt++) {
    ...
    class ThreadWicked : public kc::Thread {
     public:
      void setparams(int32_t id, kc::CacheDB* db, int64_t rnum, int32_t thnum,
                     const char* lbuf, std::pair<kc::result<bool>, errordata>* d) {
        id_ = id;
        db_ = db;
        rnum_ = rnum;
        thnum_ = thnum;
	...
	LOCK_REGISTER_THIS_THREAD();
        kc::CacheDB::Cursor* cur = db_->cursor();
        int64_t range = rnum_ * thnum_ / 2;
	for (int64_t i = 1; !err_ && i <= rnum_;) { /* no increase of i here, see next lines */
	int ctx = 0; /* initialize, needed after for loop */
	while(!err_ && i <= rnum_ && ctx < RESULT_CONTEXTS/2) {
          ...
          do {
            switch (myrand(9)) {
              ...
              case 8: {
                if (myrand(10) == 0) {
                  d_op[ctx].second.set(kc::BasicDB::Error::NOREC, db_, __LINE__, "DB::switch_rotation");
		  cur->jump(&d_op[ctx].first, &d_op[ctx].second.error, kbuf, ksiz);
		  ctx++;
                } else {
		  ...
		  // accept will return a result (future), so check that instead of directly
		  d_op[ctx].second.set(kc::BasicDB::Error::NOREC, db_, __LINE__, "Cursor::accept");
		  cur->accept(&d_op[ctx].first, &d_op[ctx].second.error, &visitor, true, myrand(2) == 0);
		  ctx++;
                  if (myrand(5) > 0) {
			  d_op[ctx].second.set(kc::BasicDB::Error::NOREC, db_, __LINE__, "Cursor::step");
			  cur->step(&d_op[ctx].first, &d_op[ctx].second.error);
			  ctx++;
		  }
                }
                break;
	  ...
	  d_op[ctx].second.set_all_ok(db_, __LINE__, "DB::switch_rotation");
	  db_->switch_rotation(&d_op[ctx].first, myrand(4) > 0);
	  ctx++;
          }
          if (i == rnum_ / 2) {
	...
	for(int c = 0; c < ctx; c++) {
          if (!d_op[c].first.get() && !d_op[c].second.check_ok()) {
            err_ = true;
          }
          d_op[c].first.reset();
	}
     ...
     private:
      ...
      const char* lbuf_;
      bool err_;
      std::pair<kc::result<bool>, errordata>* d_op;
    };
    char lbuf[RECBUFSIZL];
    std::memset(lbuf, '*', sizeof(lbuf));
    ThreadWicked threads[THREADMAX];
    if (thnum < 2) {
	std::pair<kc::result<bool>, errordata> d_op[RESULT_CONTEXTS];
      threads[0].setparams(0, &db, rnum, thnum, lbuf, d_op);
      threads[0].run();
      if (threads[0].error()) err = true;
    } else {
	std::pair<kc::result<bool>, errordata> d_op[THREADMAX][RESULT_CONTEXTS];
      for (int32_t i = 0; i < thnum; i++) {
        threads[i].setparams(i, &db, rnum, thnum, lbuf, d_op[i]);
        threads[i].start();
      }
      for (int32_t i = 0; i < thnum; i++) {
        threads[i].join();
