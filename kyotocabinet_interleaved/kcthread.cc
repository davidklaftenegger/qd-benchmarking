...
void SpinLock::lock() {
  ...
  _assert_(true);
  uint32_t wcnt = 0;
  while (!__sync_bool_compare_and_swap(&opq_, 0, reinterpret_cast<void*>(1l))) {
    if (wcnt >= LOCKBUSYLOOP) {
      Thread::chill();
    } else {
      Thread::yield();
...
#elif _KC_GCCATOMIC
  _assert_(true);
  return __sync_bool_compare_and_swap(&opq_, 0, reinterpret_cast<void*>(1l));
#else
  _assert_(true);
  ::pthread_spinlock_t* spin = (::pthread_spinlock_t*)opq_;
...
/**
 * call a delegated functor.
 * @param ptr pointer to the functor
 * @param addr ignored
 */
static void invoke_functor(void* ptr, void** addr) {
	std::function<void()>* fptr = static_cast<std::function<void()>*>(ptr);
	(*fptr)();
}

/**
 * call a delegated functor, then delete it.
 * @param ptr pointer to the functor
 * @param addr ignored
 */
static void invoke_delete_functor(void* ptr, void** addr) {
	invoke_functor(ptr, addr);
	std::function<void()>* fptr = static_cast<std::function<void()>*>(ptr);
	delete fptr;
}
 ...
 * Default constructor.
 */
RDLock::RDLock() : opq_(NULL) {
  _assert_(true);
  opq_ = ::LOCK_CREATE(nullptr);
}
 ...
 */
void RDLock::lock_delegate(std::function<void()> f) {
  std::function<void()>* fptr = new std::function<void()>(f);
  ::LOCK_DELEGATE(opq_, invoke_delete_functor, fptr);
}

void RDLock::lock_delegate(std::function<void()>* f) {
  ::LOCK_DELEGATE(opq_, invoke_functor, f);
}

/**
 * Get a reader lock.
 */
void RDLock::lock_reader() {
  _assert_(true);
  ::LOCK_READ_LOCK(opq_);
}
