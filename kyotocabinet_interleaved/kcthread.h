...
template<typename T>
class result {
	std::atomic<bool> output_set;
	T t;
	public:
		char space[80];
		result() : output_set(false) {}
		T get() {
			while(!output_set) {
				__sync_synchronize();
			}
		...
		void set(T p) { t = p; output_set = true; }
		void reset() {
			while(!output_set) {
				__sync_synchronize();
			}
...
template<int MAXSIZE, typename Fun, typename R, typename... Ts>
class delegatable_function {
	public:
	static_assert(sizeof(std::atomic<bool>)+sizeof(std::atomic<Fun*>) + sizeof(result<R>) + sizeof(std::tuple<Ts...>) < MAXSIZE, "supplied types too big");
	std::atomic<Fun*> function;
	result<R>* output;
	std::tuple<Ts...> input;

	public:
		delegatable_function(Fun f, result<R>* r, Ts... inputs) : function(nullptr), output(r), input(inputs...) {
			function.store(f);
		}
		delegatable_function(delegatable_function<MAXSIZE, Fun, R, Ts...>& rhs) : function(nullptr), output(rhs.output), input(rhs.input) {
			function.store(rhs.function.load());
		}
		delegatable_function(delegatable_function<MAXSIZE, Fun, R, Ts...>&& rhs) : function(nullptr), output(rhs.output), input(rhs.input) {
			rhs.output = nullptr;
			rhs.input = std::tuple<Ts...>();
			function.store(rhs.function.load());
			rhs.function.store(nullptr);
		}

		void run() {
			while(!function.load());
			auto f = function.load();
...
template<int MAXSIZE, typename Fun, typename R, typename... Ts>
union delegatable_union {
	delegatable_function<MAXSIZE, Fun, R, Ts...> fun;
	char buffer[MAXSIZE];
	delegatable_union(Fun f, result<R>* r, Ts... inputs) : fun(f, r, inputs...) {}
	delegatable_union(delegatable_union<MAXSIZE, Fun, R, Ts...>& rhs) : fun(std::forward<delegatable_union<MAXSIZE, Fun, R, Ts...>>(rhs.fun)) {}
	delegatable_union(delegatable_union<MAXSIZE, Fun, R, Ts...>&& rhs) : fun(std::forward<delegatable_union<MAXSIZE, Fun, R, Ts...>>(rhs.fun)) {
		rhs.fun = Fun();
	}
...
template<typename Fun, typename R, typename... Ts>
using delegatable = delegatable_union<128, Fun, R, Ts...>;
/**
 * call a delegated package.
 * @param ptr pointer to the std::packaged_task
 * @param addr ignored
 */
template<typename T>
void invoke_package(void* ptr, void** addr) {
//	std::cout << "PACKAGE INVOKED\n";
	std::packaged_task<T()>* pptr = static_cast<std::packaged_task<T()>*>(ptr);
	(*pptr)();
}
/**
 * call a delegated package.
 ...
 */
template<typename T>
void invoke_delete_package(void* ptr, void** addr) {
	std::cout << "PACKAGE INVOKED AND DELETED\n";
	std::packaged_task<T()>* pptr = static_cast<std::packaged_task<T()>*>(ptr);
	(*pptr)();
	delete pptr;
...
template<typename T>
void invoke_delegatable(void* ptr, void** addr) {
//	std::cout << "NEW STYLE DELEGATION\n";
	T* t = static_cast<T*>(ptr);
	t->run();
}
template<typename T>
void invoke_delegatable_del(void* ptr, void** addr) {
	T* t = static_cast<T*>(ptr);
	t->run();
	t->~T();
}
...
/**
 * Reader-Delegate locking device.
 */
class RDLock {
 public:
  /**
   ...
   */
  void lock_delegate(std::function<void()> f);
  void lock_delegate(std::function<void()>* f);
  template<typename T>
  void lock_delegate(std::packaged_task<T()>* f);
  template<typename T>
  void lock_delegate(T* t);
  template<typename T>
  void lock_delegate_del(T* t);
  /**
   * Get a reader lock.
   */
  void lock_reader();
 ...
 private:
  ...
  /** Opaque pointer. */
  LOCK_DATATYPE_NAME* opq_;
};
template<typename T>
void RDLock::lock_delegate(std::packaged_task<T()>* f) {
  ::LOCK_DELEGATE(opq_, invoke_package<T>, f);
}

template<typename T>
void RDLock::lock_delegate(T* t) {
  ::LOCK_DELEGATE(opq_, invoke_delegatable<T>, t);
}
template<typename T>
void RDLock::lock_delegate_del(T* t) {
  ::LOCK_DELEGATE(opq_, invoke_delegatable_del<T>, t);
}
/**
...
/**
 * Scoped read-delegate-exclusive locking device.
 */
class ScopedRDLock {
 public:
  /**
   * Constructor.
   * @param rdxlock an rdxlock to lock the block.
   * @param type R for reader, W for writer lock, or X for exclusive lock.
   */
  explicit ScopedRDLock(RDLock* rdlock) : rdlock_(rdlock) {
    _assert_(rdlock);
    rdlock_->lock_reader();
  }
  /**
 ...
 private:
  /** Dummy constructor to forbid the use. */
  ScopedRDLock(const ScopedRDLock&);
  /** Dummy Operator to forbid the use. */
  ScopedRDLock& operator =(const ScopedRDLock&);
  /** The inner device. */
  RDLock* rdlock_;
};
